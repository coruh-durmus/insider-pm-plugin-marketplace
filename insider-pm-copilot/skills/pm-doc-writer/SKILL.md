...
- **Confluence — Read:** Via `mcp__ekb__jira` (ekb MCP). You can search and read all pages and spaces.
...
- **Jira, Codebase, Academy, Slack, Hop:** Accessed via the `pm-internal-knowledge` skill at `${CLAUDE_PLUGIN_ROOT}/skills/pm-internal-knowledge/SKILL.md` (read-only).
...
1. **Config exists:** Read `${CLAUDE_PLUGIN_ROOT}/config/team-config.json`. Parse the `docs` section to get `docs.confluence_space`, `docs.confluence_parent_page_id`, `docs.jira_projects`, `docs.trigger`, and `docs.review`. If the file does not exist or the `docs` section is missing/null, tell the PM:
   > "Please run `/setup` first and configure the Documentation section."
   Stop here.
...
Invoke the `pm-internal-knowledge` skill (at `${CLAUDE_PLUGIN_ROOT}/skills/pm-internal-knowledge/SKILL.md`) with the context seed. The knowledge hub searches:
- **Confluence** (ekb) — existing related documentation
...
Use `mcp__ekb__jira` to search for existing Confluence pages under the configured parent page that match the feature being documented (by title similarity or content match).
...
Use `mcp__ekb__jira` to search for and read **5-10 existing child pages** under the PM's configured parent page.
...
**If Atlassian MCP is available, double confirmation required:**
...
- **Config required.** If `team-config.json` is missing or `docs` section is null, stop and ask the PM to run `/setup`.
- **Delegate context gathering.** Use the `pm-internal-knowledge` skill for all source querying. Do not duplicate its logic.
