...
1. **Internal knowledge** (the `pm-internal-knowledge` skill at `${CLAUDE_PLUGIN_ROOT}/skills/pm-internal-knowledge/SKILL.md`): Search for related Confluence specs, Jira tickets, codebase insights, Academy guides, and Slack discussions relevant to the task.

2. **Competitor intel** (the `competitor-intelligence` skill at `${CLAUDE_PLUGIN_ROOT}/skills/competitor-intelligence/SKILL.md`): Only if the task has competitive relevance (mentions a competitor, competitive feature, or market positioning).
