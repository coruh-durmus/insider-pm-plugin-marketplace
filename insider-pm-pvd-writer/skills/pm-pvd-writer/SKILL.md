...
### Competitor Research (via insider-competitor-intel)
Invoke the `insider-competitor-intel` skill to research how competitors handle the same feature area. This provides:
...
If the PM provided a reference PVD, read it from Confluence using Atlassian MCP tools. Analyze:
...
Read the company-wide PVD template from Confluence. The template is the "Product Requirements Documents (PRD)" page under the Product Knowledge Base. Use Atlassian MCP tools to read this page and extract its section structure.
...
2. **If PM chooses A (Direct Write), double confirmation:**
   > "This will create a new page titled '[PVD Title]' under '[Team PVD Parent Page]' in Confluence. Are you sure? (yes/no)"

3. If confirmed, use Atlassian MCP tools to create the page under the team's configured `pvd_parent_page_id`.
